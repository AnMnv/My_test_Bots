...
loading_sti = 'CAACAgIAAxkBAAEItPNkRp2n4RlDpujrFbbn1PIuRZ-WtgACDgADr8ZRGrdbgux-ASf3LwQ'  # id стикерa (@idsticker_bot)
...
#info_text = f"""<code>Send a photo 🖼</code>"""
#################
...
async def start(message):
	...
	start_pressed.set(str(f'{message.from_user.id}'), str(f'@{message.from_user.username} | {message.from_user.first_name} {message.from_user.last_name}'))
	start_pressed.dump()
#################
...
async def get_location(message: types.Message, state: FSMContext):
	...
	if message.location is not None:
		...
		location_button = KeyboardButton(text="share_location_button", request_location=True)
		markup = ReplyKeyboardMarkup(keyboard=[[location_button]],one_time_keyboard=True, resize_keyboard=True)
...
async def get_text(message: types.Message, state: FSMContext):
	...
	if state_caption[message.from_user.id] == button_skip:
		...
		b2 = InlineKeyboardButton("👍 " + str(message_ratings[message_id]['likes']), callback_data=f"like,{data['location']}")
		b3 = InlineKeyboardButton("👎 " + str(message_ratings[message_id]['dislikes']), callback_data=f"dislike,{data['location']}")
		markup.add(b2,b3,b1)
		data['photo'].seek(0)
		...
		usersID_counter = usersID.get(str(f'@{message.from_user.username} {message.from_user.id}'))
		if usersID_counter == False:
			usersID.set(str(f'@{message.from_user.username} {message.from_user.id}'), '1')
		else:
			usersID.set(str(f'@{message.from_user.username} {message.from_user.id}'),str(int(usersID_counter) + 1))
		usersID.dump()
		...
		try:
			...
			data = await state.get_data()
			data['photo'].seek(0)
			await bot.send_photo(channel_NSFW, data['photo'],caption=f"{state_caption[message.from_user.id]}", parse_mode='html', reply_markup=markup)# change to main channel
			thanks_photo_posted = messages["thanks_photo_posted"][language[message.from_user.id]]
			await message.answer(thanks_photo_posted)
			# дублируем в channel_history 
			data = await state.get_data()
...
async def update_message_rating(message: types.Message, id):
	...
	like_button = InlineKeyboardButton("👍 " + str(rating['likes']), callback_data=f'like,{id}')
	dislike_button = InlineKeyboardButton("👎 " + str(rating['dislikes']), callback_data=f'dislike,{id}')
	markup = InlineKeyboardMarkup(row_width=2)
...
async def qqq(message):
    ...
    b1 = types.InlineKeyboardButton("👉 Send photo 👈", url="https://t.me/found_it_outside_bot")
    markup.add(b1)
